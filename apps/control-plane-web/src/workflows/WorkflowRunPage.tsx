...
export const WorkflowRunPage: FC = () => {
  ...
  const graph = useMemo(() => (run ? buildWorkflowGraph(run.definition, run.steps) : null), [run]);
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 2 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
        <Button component={Link} to="/workflows" size="small" variant="text">
          ← Workflows
        </Button>
        <Typography variant="h6">{run.definition.name}</Typography>
        <Chip label={run.status} color={RUN_STATUS_COLOR[run.status]} size="small" />
        {run.iteration > 0 && <Chip label={`iteration ${run.iteration}`} size="small" variant="outlined" />}
        {run.prUrl && (
          <MuiLink href={run.prUrl} target="_blank" rel="noreferrer" sx={{ ml: 'auto' }}>
            View PR
          </MuiLink>
        )}
      </Box>
