...
// Re-exported for callers that still reference the runner launcher path by name.
export { START_SCRIPT };
...
interface AgentDriverDeps {
  db: Db;
  eventStore: EventStore;
  eventBus: EventBus;
  exec: ContainerExec;
  // Tear down the box once the run ends — unlike interactive sessions, a headless
  // runner has no human to dismiss it, and the old runner process used to self-exit.
  retire: (containerId: string) => Promise<void>;
}

export const createAgentDriver = (deps: AgentDriverDeps) => {
  ...
  // never push a PR — the workflow driver owns the shared branch and pushes once at run end.
...
export type AgentDriver = ReturnType<typeof createAgentDriver>;
