 ...
 *  `userKey` (the caller's displayName today). One row per user; reads fall back
...
export const createCanvasLayoutService = (deps: CanvasLayoutServiceDeps) => ({
  ...
  get: async (userKey: string): Promise<CanvasLayout> => {
    const [row] = await deps.db.select().from(canvasLayouts).where(eq(canvasLayouts.userKey, userKey)).limit(1);
  ...
  set: async (userKey: string, layout: CanvasLayout): Promise<void> => {
    const valid = canvasLayoutSchema.parse(layout);
    await deps.db
      .insert(canvasLayouts)
      .values({ userKey, layout: valid })
      .onConflictDoUpdate({ target: canvasLayouts.userKey, set: { layout: valid, updatedAt: new Date() } });
  },
});
