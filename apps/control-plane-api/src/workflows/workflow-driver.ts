 ...
 */
export const createWorkflowDriver = (deps: WorkflowDriverDeps) => {
  ...
  // orchestrator relies on. The runner's start-agent prepends SOUL.md to this.
  ...
  const drive = async (
    ...
    try {
      ...
      for (;;) {
        ...
        const { exitCode } = await deps.agentDriver.execStep({
          taskId,
          containerId,
          manifest,
          sessionDir: dir,
        ...
        if (step.kind === WorkflowStepKind.VALIDATION) {
          if (!opsContainerId) {
            ({ containerId: opsContainerId } = await deps.spawner.spawn({
              ...
              sessionDir: dir,
              userEnv,
              runnerImage: deps.config.runnerImage,
            }));
          }
...
export type WorkflowDriver = ReturnType<typeof createWorkflowDriver>;
