...
import { createWorkflowDriver, type RunFiles } from './workflow-driver';
...
const definition = {
  ...
  maxIterations: 3,
  steps: [
    { key: 'plan', name: 'Plan', kind: WorkflowStepKind.WORK, runnerImage: 'w', goal: 'plan' },
    { key: 'implement', name: 'Implement', kind: WorkflowStepKind.WORK, runnerImage: 'w', goal: 'build' },
    { key: 'validate', name: 'Validate', kind: WorkflowStepKind.VALIDATION, runnerImage: 'w', goal: 'check', onFailureGoTo: 'implement' },
  ],
};

// Build a driver over the pg-mem db with fully faked containers/files. `verdicts`
...
const setup = async (opts: {
  ...
  // One spawner shared by the step seam (sessionService) and the driver's ops
  ...
  const driver = createWorkflowDriver({
    ...
    spawner: spawner as never,
    sessionService,
    agentDriver: { execStep: opts.execStep ?? (async () => ({ exitCode: opts.stepExit ?? 0 })) },
    exec: {
...
describe('workflow-driver', () => {
  ...
  it('passes the full resolved user env to each step, not just GITHUB_TOKEN', async () => {
    ...
    // stored .env — the old workflow-driver built step env as { GITHUB_TOKEN } only.
