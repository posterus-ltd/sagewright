...
## ADR-0001 — Harness-agnostic runner via the "remote box" model
...
The agent harness (opencode) was hardwired into the runner in three places: `runner/Dockerfile`
installed `opencode-ai` and baked its config, and `runner/src/main.ts` constructed `OpencodeHarness`
...
We want the **runner Dockerfile to be the single place** a user installs and authenticates a harness
...
model: the runner is a remote box you connect to and run any agent inside, as if over SSH; the agent
behaves exactly as if a human had connected and launched it.

### Decisions

1. **Runner = generic box; harness chosen and configured in `runner/Dockerfile`.** A single,
...
2. **Predefined start-script contract.** The runner image exposes a known executable (the start
...
3. **Control plane is thin infrastructure: spawn + access + env-injection.** Its only runner-facing
   primitives are (a) spawn a runner container, (b) provide terminal/exec access into it, and
...
4. **Transport is a docker-exec PTY, not literal SSH.** The control plane and runners are always
...
5. **One terminal channel, two drivers.** The same PTY into the runner is driven either by a human
...
- Bring any harness by editing only `runner/Dockerfile` and the start script; no control-plane or web
  changes are required.
- The runner's Node orchestration app and the `libs/harness` adapter are removed; push/PR logic moves
  into the control plane and the runner→control-plane HTTP callback (and its runner token) disappears.
